            ...
            children: [
              ...
              Column(children: [
                RadioListTile(
                  ...
                  groupValue: onePhotoMemoTempLabeler,
                  onChanged: (value) {render(() => onePhotoMemoTempLabeler = value);} //con.setLabeler(value),
                ),
                RadioListTile(
              ...
              ],) : SizedBox(height:1.0,),// : SizedBox(height:1.0,),
              // editMode ? con.photoFile == null ? SizedBox(height:1.0,) : Column(children:con.getRadioTiles(onePhotoMemoTemp.labeler)) : SizedBox(height:1.0,),
              // editMode ? Column(children:con.getRadioTiles(onePhotoMemoTemp.labeler),) : SizedBox(height:1.0,),
              Constant.DEV
                  ? Text(
              ...
              Constant.DEV
                  ? Text(
                      onePhotoMemoTemp.imageLabels.toString().replaceAll(", ", " "),//.join(' | '),
                    )
                  : SizedBox(
                      height: 1.0,
                    ),
...
class _Controller {
  ...
  void update() async {
    ...
    try {
      ...
      if (photoFile != null) {
        if(state.onePhotoMemoTempLabeler == null) { 
          MyDialog.circularProgressStop(state.context);
          return MyDialog.info(context: state.context, title: "No Labeler Chosen", content: "You must select an image Labeler!!");
        }
        ...
        if(state.onePhotoMemoTempLabeler != null) {
          if(state.onePhotoMemoTempLabeler == MLAlgorithm.MLLabels) {
          ...
          imageLabels =
            await FirebaseController.recogniseText(photoFile);
        }
      } else {
        if(state.onePhotoMemoTemp.labeler == MLAlgorithm.MLLabels.toString()) {
          state.render(() => state.progressMessage = 'ML Image Labeler Started!');
          imageLabels = 
            await FirebaseController.getImageLabels(photoFile: photoFile);
        } else {
        ...
        state.onePhotoMemoTemp.labeler = state.onePhotoMemoTempLabeler.toString(); 
        updateInfo[PhotoMemo.LABELER] = state.onePhotoMemoTemp.labeler;
        ...
        updateInfo[PhotoMemo.IMAGE_LABELS] = state.onePhotoMemoTemp.imageLabels;
      }
